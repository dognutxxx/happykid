...
const FlightApp = () => {
  ...
  const onValuesChange = (value) => {
    ...
    fetch(`https://testnets-api.opensea.io/api/v1/assets?owner=${account}&token_ids=${value.nftID}&asset_contract_address=${collectionContract}&order_direction=desc&offset=0&limit=20&include_orders=false`, options)
      .then(response => response.json())
      .then(response => {
    ...
    <>
      <div className="b" >
        {
          data
            ...
            .map((data, index) =>
              contract === collectionContract ? <img key={index} src={data.image_thumbnail_url} alt={"img"} width="300" height="300" /> : null)
        }
      </div>

      <div className="b">
        {status === 0 ?
          <p>ตั๋วใบนี้ถูกใช้งานเรียบร้อยแล้ว</p>
          : null
        }
      </div>
      ...
      <div background-color= "red">
        <div style={{ display: 'flex', justifyContent: 'center', color: 'red' }}>
          {account === '' ? "Please connect Metamask (Ethereum Chain)" : `Your address is : ${account}`}
        </div>
        <hr />

        <div className="d">
          {contract === collectionContract ? `You have amount : ${data.length} NFT Ticket ` : "No have NFT Ticket"}
        </div>
        ...
        {contract === collectionContract ?
          <Form form={form} onFinish={onFinish} onValuesChange={onValuesChange} >
            ...
            <Form.Item label="NFT ID" name="nftID" rules={[
              {
                required: true,
            ...
            <Form.Item label="TripType" name="tripType" rules={[
              {
                required: true,
                message: 'Please input your TripType!',
              },
            ]}>
              <Select placeholder="Select a TripType" onChange={e => setTripType(e)}>
            ...
            <Form.Item label="Passenger(adult)" name="adult" rules={[
              {
                required: true,
            ...
            <Form.Item label="Passenger(child)" name="child" rules={[
              {
                required: true,
            ...
            <Form.Item label="Passenger(infant)" name="infant" rules={[
              {
                required: true,
            ...
            <Form.Item label="Class" name="svcClass" rules={[
              {
                required: true,
            ...
            <Form.Item label="Departure" name="originCode" rules={[
              {
                required: true,
            ...
            {tripType === "R" ?
              <Form.Item label="Arrival" name="destinationCode" rules={[
                {
                  required: true,
            ...
            <Form.Item label="Language" name="languageCode" rules={[
              {
                required: true,
            ...
            <Form.Item label="Airlines" name="airlines" rules={[
              {
                required: true,
            ...
            <Form.Item label="Departure Date" name="departDate" rules={[
              {
                required: true,
                message: 'Please input your Departure Date!',
              },
            ]}>
              <DatePicker disabledDate={date => !date || date.isAfter(`${id.bookingFinishDate}`) || date.isSameOrBefore(`${id.bookingStartDate}`)}
                format={dateFormat} defaultPickerValue={moment("2022-09-01")}
                placeholder="Please select date"
              />
            </Form.Item>

            <Form.Item label="Arrival Date" name="returnDate" rules={[
              {
                required: true,
              ...
              <DatePicker disabledDate={date => !date || date.isAfter(`${id.bookingFinishDate}`) || date.isSameOrBefore(`${id.bookingStartDate}`)}
                format={dateFormat} defaultPickerValue={moment("2022-10-01")}
                placeholder="Please select date"
              />
            </Form.Item>
          ...
          </Form> : <a href="http://localhost:3003/" target="_blank"> <button>Mint</button> </a>
        }
